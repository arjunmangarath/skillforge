...
import {
  LayoutDashboard, Map, Brain, Sparkles, Gamepad2, Lock, LogOut,
} from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-16 lg:w-56 flex flex-col bg-surface-2 border-r border-border z-40">
      ...
      {/* Bottom: activity log + sign out */}
      <div className="px-2 pb-4 border-t border-border pt-3 space-y-1">
        <ActivityLogPanel />
        <button
          onClick={handleSignOut}
          className="flex items-center gap-3 px-3 py-2.5 w-full rounded-input text-text-muted hover:text-red-400 hover:bg-red-500/10 transition-colors"
        >
